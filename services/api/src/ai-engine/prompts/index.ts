// ─────────────────────────────────────────────────────────────────────────────
// services/api/src/ai-engine/prompts/index.ts
...
export function plannerPrompt(input: MealPlanInput): string {
  const inventory = input.availableInventory
    .map((i: InventoryItem) => `${i.name}: ${i.quantityKg}kg`)
    .join(", ");
...
export function predictionPrompt(input: ParticipationInput): string {
  const recentAttendance = input.historicalAttendance
    .slice(-10)
    .map((r: AttendanceRecord) => `${r.date}: ${r.present}/${r.total}`)
    .join(", ");

  const recentOptOuts = input.mealOptOutData
    .slice(-5)
    .map((o: OptOutRecord) => `${o.date}: ${o.count} opt-outs`)
    .join(", ");
...
Upcoming holidays near this date: ${input.holidayData.map((h: HolidayRecord) => `${h.date} - ${h.name}`).join(", ") || "none"}
...
export function wastagePrompt(input: WasteInput): string {
  const prepared = input.preparedMeals.map((m: PreparedMealRecord) => `${m.mealName}: ${m.quantityKg}kg`).join(", ");
  const served = input.servedMeals.map((m: ServedMealRecord) => `${m.mealName}: ${m.quantityKg}kg`).join(", ");
  const historical = input.historicalWaste
    .slice(-7)
    .map((w: WasteRecord) => `${w.date} ${w.mealName}: ${w.wasteKg}kg`)
    .join(", ");
...
export function holidayPrompt(input: HolidayPlanInput): string {
  const festivals = input.festivalList
    .map((f: Festival) => `${f.date}: ${f.name} (${f.region})${f.dietaryConsiderations ? ` - dietary note: ${f.dietaryConsiderations}` : ""}`)
    .join("\n");

  const calendar = input.schoolCalendar
    .filter((e: SchoolCalendarEvent) => e.affectsMeals)
    .map((e: SchoolCalendarEvent) => `${e.date}: ${e.event}`)
    .join("\n");
...
Current meal schedule dates: ${input.currentMealSchedule.map((d: DayMealPlan) => d.date).join(", ")}
...
export function plannerAnalysisPrompt(planData: any): string {
  const serialized = JSON.stringify(planData, null, 2);
  return `
Analyze the following weekly meal plan for an Indian school.
The plan consists of Mon-Sun with Morning, Afternoon, Evening, and Dinner meals.
Some slots might be marked as "Holiday (Closed)" or "No Meal", which means no meal is served.

Weekly Plan:
${serialized}

Please analyze this schedule for nutritional adequacy based on ICMR guidelines. Identify deficiency gaps (e.g. lack of protein, iron, or vitamins). Suggest specific, cost-effective Indian ingredients to add as a snack or adjust in other meals to bridge these nutritional gaps.

You MUST respond with this exact JSON schema:
{
  "warnings": ["<string warning about deficiency, lack of meals, or budget issues>"],
  "suggestions": [
    {
      "day": "<string Day of week>",
      "mealTime": "Morning|Afternoon|Evening|Dinner|Snacks",
      "foodSuggestion": "<string ingredient or meal to add/modify>",
      "reason": "<string explanation of which nutrient this covers>"
    }
  ],
  "overallAnalysis": "<string summary of the analysis and nutritional state>"
}
`.trim();
}
